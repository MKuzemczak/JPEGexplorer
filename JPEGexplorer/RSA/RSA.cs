...
namespace JPEGexplorer.RSA
{
    public static class RSAService
    {
        ...
        public static byte[][] code(byte[] data, long wykladnik, long modul)
        {
            ...
            //byte[] returned = new byte[data.Length * BitConverter.GetBytes(doKodowania[0]).Length];
            //int cntr = 0;
            //foreach (var value in doKodowania)
            //{
            //    byte[] dodawane = BitConverter.GetBytes(value);
            //    dodawane.CopyTo(returned, cntr);
            //    cntr += dodawane.Length;
            //}
